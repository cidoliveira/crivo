...
O `facebook/bart-large-mnli` é um modelo pré-treinado para classificação zero-shot via inferência de linguagem natural (NLI). Ele avalia se um texto é consistente com cada rótulo candidato usando a relação de implicação lógica — entailment vs. contradiction. Não precisa de fine-tuning nem de dados rotulados.

O modelo foi treinado em inglês (MNLI dataset). Para classificar emails em português, usamos labels em inglês (`productive` / `unproductive`) com um hypothesis template contextualizado (`"This email is a {} business communication."`), fazendo a ponte entre o texto em PT-BR e a capacidade de NLI do modelo em inglês. Na prática, isso produz classificações com alta confiança (>90%) para emails de negócio claros, superando abordagens com labels em português ou modelos multilíngues testados.
